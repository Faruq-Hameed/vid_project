...
moviesRouter.post('/', (req, res) => {
    ...
    const genre = genres.find(genre => genre.name === req.body.genre);
    if (!genre) return res.status(404).send('genre not found');
    const newMovie = req.body
    newMovie.id = movies.length + 1
    movies.push(newMovie)
    genre.availableMovies += 1 //increment the movie to the genre
    res.status(200).json({ newMovie })

})

moviesRouter.put('/:movieId/:adminId', (req, res) => {
    if (req.body.adminId !== "1") return res.status(401).send('unauthorized user');
    const movie = getById(movies, req.params.movieId)
    const movieIndex = getIndexById(movie, req.params.movieId)
    if (!movie) return res.status(404).send('movie not found');
    ...
    updatedMovie.genre = movie.genre // movie genre can't be changed
    movies.slice(movieIndex, 1, updatedMovie)
})
...
moviesRouter.delete('/:movieId/:adminId', (req, res) => {
    ...
    const movie = getById(movies, req.params.movieId)
    const movieIndex = getIndexById(movie, req.params.movieId)
    if (!movieToDelete) return res.status(404).send('movie not found');
...
function validateSchema(req) {
    const schema = Joi.object({
        title, genre, producer: Joi.string().min(3).max(24).required(),
